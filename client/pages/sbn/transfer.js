...
class SBNTransfer extends Component {
  ...
  onSubmit = async () => {
    try {
      ...
      const { accounts, selectedSBN, unitAmount, selected_receiver } =
        this.state;

      const tx = await selectedSBN.methods
        .transfer(selected_receiver["account"], unitAmount)
        .send({
          from: accounts[0],
        });
      ...
      this.setState({
        positiveMessage:
          "Transfer completed! Transaction hash: " + tx.transactionHash,
      });
    } catch (e) {
      this.setState({
  ...
  checkForm = () => {
    ...
    const { CBDC_Dapps, accounts, selectedSBN, unitAmount, selected_receiver } =
      this.state;
  ...
  renderModal() {
    ...
    const { unitAmount, selected_receiver, selected_SBN_detail } = this.state;
    ...
    return (
      ...
      >
        ...
        <Modal.Content>
          <Modal.Description>
            <Header>Transfer SBN Detail</Header>
            <Table celled>
              <Table.Body>
                <Table.Row>
                  <Table.Cell>
                ...
                <Table.Row>
                  ...
                  <Table.Cell>
                    {`${unitAmount} unit of ${selected_SBN_detail["code"]}`}
                  </Table.Cell>
                </Table.Row>
              </Table.Body>
            </Table>
      ...
      <>
        <Layout>
          ...
          <Form error={!!this.state.errorMessage}>
            <Form.Field>
              <Header as="h3"> Choose SBN to transfer:</Header>
            ...
            <Form.Field>
              <Header as="h3"> Number of SBN unit to transfer:</Header>
